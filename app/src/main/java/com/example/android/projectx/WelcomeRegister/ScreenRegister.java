...
public class ScreenRegister extends AppCompatActivity {
    ArrayList<ModelPhnNumber> list;
    String twoLettersCountryCode;
    Button button;
    Spinner countryNameSpinner, flagSpinner;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //bind view and objects
        setContentView(R.layout.activity_register_screen);
        countryNameSpinner = findViewById(R.id.country_name_spinner);
        button = findViewById(R.id.continue_button2);
        phoneInputLayout = findViewById(R.id.phone_number_input);

        //create builder object
        builder = new AlertDialog.Builder(ScreenRegister.this);

        //dialog wont get cancelled when touched outside of the screen
        builder.setCancelable(false);

        twoLettersCountryCode = getUserCountry(this).toUpperCase();

        //get response from server, add in list,check against telephonymanager and set two spinners
//        getNotice();

        int index=0;
        for(int i=0;i<countries.COUNTRIES.size();i++){
            countryNameList.add(getCountry().get(i).getName());
            if(twoLettersCountryCode.equalsIgnoreCase(getCountry().get(i).getCode())){
                index=i;
            }
        }
        ...
        clickSpinner(countryNameSpinner);

        phoneInputLayout.setDefaultCountry(twoLettersCountryCode);
        ...
        button.setOnClickListener(new View.OnClickListener() {
                                      ...
                                      public void onClick(View view) {
                                          ...
                                          if (phoneInputLayout.isValid()) {
                                              showValidNumberAlertDialog();
                                          } else {
                                              showInvalidNumberAlertDialog();
                                          }
                                      }
                                  }
        );
    ...
    public void getNotice() {
        ...
        Call<List<ModelPhnNumber>> call = api.getJson(); //api.getJson provides data to call object list
        call.enqueue(new Callback<List<ModelPhnNumber>>() { //enqueque requests data
            @Override
            public void onResponse(Call<List<ModelPhnNumber>> call, Response<List<ModelPhnNumber>> response) { // either positive or negative response
                if (response.isSuccessful()) {
                    int getIndex = 0;
                    list = new ArrayList<>(); //define a list object                              //data is in response

                    for (int i = 0; i < response.body().size(); i++) {
                        //add country name,dial code and country code models into list
                        list.add(response.body().get(i));
                        //add only country names into the list
                        countryNameList.add(response.body().get(i).getName());  // adding data into the list
                    }
                    //define and set spinner adapter
                    ArrayAdapter<String> adapter = new ArrayAdapter<String>(ScreenRegister.this,
                            android.R.layout.simple_list_item_1, countryNameList);
                    countryNameSpinner.setAdapter(adapter);

                    //check two letter country code obtained from telephonmetry and use it to set adapter
                    for (int i = 0; i < countryNameList.size(); i++) {
                        //store NP in name..
                        String name = response.body().get(i).getCode();
                        //check NP from response with NP from getUserCountry func
                        if (twoLettersCountryCode.equalsIgnoreCase(name)) {
                            getIndex = i;
                        }
                    }

                    //country name spinner sets Nepal as choice.
                    countryNameSpinner.setSelection(getIndex);
            ...
            @Override
            public void onFailure(Call<List<ModelPhnNumber>> call, Throwable t) {
                Toast.makeText(ScreenRegister.this, "No Internet Connection!!!", Toast.LENGTH_SHORT).show();
            }
        });
    }
...
//                countryCode.setText(list.get(i).getDialCode());
                phoneInputLayout.setDefaultCountry(getCountry().get(i).getCode());
            }
    ...
    public void showValidNumberAlertDialog() {
    ...
    public void showInvalidNumberAlertDialog() {
